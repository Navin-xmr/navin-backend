...
const EnvSchema = z
  .object({
    ...
    // SOROBAN_ADAPTER selects between simulated (in-memory/Horizon manage-data) and Soroban contract invocation.
    ...
    if (data.SOROBAN_ADAPTER === 'soroban') {
      if (!data.SOROBAN_RPC_URL) {
        ctx.addIssue({
          ...
          message: 'SOROBAN_RPC_URL is required when SOROBAN_ADAPTER is "soroban"',
        });
      }
      if (!data.ESCROW_CONTRACT_ID) {
        ctx.addIssue({
          ...
          message: 'ESCROW_CONTRACT_ID is required when SOROBAN_ADAPTER is "soroban"',
        });
      }
    }
  });
